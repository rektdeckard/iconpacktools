...
class FilterView : View("Filters") {
    private val controller: FilterController by inject()
...
//                                    }
                            ...
                            vbox(spacing = 16) {
                                ...
                                hbox(spacing = 32) {
                                    fieldset("Output Options") {
                                        field {
                                            checkbox("Generate appmap.xml").bind(controller.generateAppMap)
                                        }
                                        field {
                                            checkbox("Generate theme-resources.xml").bind(controller.generateThemeResources)
                                        }
                                        field {
                                            ...
                                            vbox(spacing = 8) {
                                                togglegroup {
                                                    ...
                                                    radiobutton("Specify directory") {
                                                        action { controller.useSpecifiedDirectory() }
                                                    }.setOnMouseClicked { controller.chooseDestination() }
                                                }
                                            }
                                        }
                                    }.addClass(fieldLabel)
                ...
                buttonbar {
                    button("Generate") {
                        enableWhen(controller.filterFile.isNotNull
                                .and(controller.generateAppMap.or(controller.generateThemeResources))
                                .and(controller.validDestination).and(controller.validFile))
        ...
        bottom = stackpane {
            ...
            label {
                ...
                toggleClass(statusSuccess, controller.statusComplete)
            }
        }
    }
